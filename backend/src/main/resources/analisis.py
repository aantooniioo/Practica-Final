...
try:
    # Ruta donde se guardarán las imágenes (dentro del contenedor)
    ruta_graficas = "/app/src/main/resources/static/graficas"

    # Crear carpeta si no existe
    os.makedirs(ruta_graficas, exist_ok=True)

    # Verificar que la carpeta es escribible
    if not os.access(ruta_graficas, os.W_OK):
        print(f"Error: No se puede escribir en {ruta_graficas}", file=sys.stderr)
        sys.exit(1)

    # Conexión a la base de datos (para Docker)
    conexion = mysql.connector.connect(
        host="practica_final-mysql-1",
        user="root",
        password="root",
        database="practica"
    )
